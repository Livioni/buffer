...
## prepare data
fields = ['Timestamp', 'File Name', 'File Size(Byte)','Canvas Size', 'Batch Size', 'Image Number', 'Canvas Efficiency', 'Inference Time (ms)', 'Transmission Time (ms)', 'Latency (ms)', 'Idle Cost (CNY)', 'Trigger Cost(CNY)']
...
for key,value in minibatch.items():
    ...
    cost = Ali_function_cost(Time=time_taken, Mem=4, CPU=2, GPU=6)
    ...
    data_frame = pd.DataFrame([[start_time, file, total_file_size,canva_size, len(canvas),\
                                len(image_path_list),round(canvas_efficency,5), round(time_taken*1000,5),\
    ...
    print('File: ', file, 'Inference Time: ', round(time_taken,5),'Transmission Time:',transmission_time,'Latency:',latency, 'Cost', cost)
    queue.clear()
    time.sleep(1)
