...
class BinPack:
    ...
    def visualize_packing(self,result):
        colors = plt.cm.tab20.colors
        colors_index = 0
        for i, bin in enumerate(result.values()):
            fig, ax = plt.subplots()   
            ax.set_title('Bin Packing Visualization')
            if bin == []:
                continue
            for item in bin['items']:
                rect = plt.Rectangle((item[0].x, item[0].y), item[1].width, item[1].height, color=colors[colors_index % len(colors)])
                ax.add_patch(rect)
                colors_index += 1
            plt.xlim(0, self.bin_size[0])
            plt.ylim(0, self.bin_size[1])
            plt.gca().set_aspect('equal', adjustable='box')
            plt.savefig('bin_packing' + str(i) + '.png')
        return
